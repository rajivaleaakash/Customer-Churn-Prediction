...
def create_features(df):
    ...
    try:
        ...
        if 'tenure' in df_copy.columns:
            ...
            logger.warning("'tenure' column not found, skipping tenure features")
        ...
        if 'contract' in df_copy.columns:
            ...
            logger.warning("'contract' column not found, skipping contract features")
        ...
        if available_service_cols:
            ...
            logger.info(f"Created service bundle score using {len(available_service_cols)} columns")
        else:
            logger.warning("No service columns found for bundle score")
        ...
        if 'totalcharges' in df_copy.columns and 'tenure' in df_copy.columns:
            ...
            logger.info("Created charges per tenure feature")
        ...
        logger.info(f"Feature engineering completed. Shape: {df_copy.shape}")
        return df_copy
...
def preprocess_features(df, target_col='churn', is_training=True):
    ...
    try:
        logger.info(f"Starting feature preprocessing. Input shape: {df.shape}")
        ...
        logger.info(f"Dropped {initial_rows - len(pre_df)} rows with missing values")
        ...
        if 'totalcharges' in pre_df.columns:
            ...
            if missing_charges > 0:
                ...
                logger.info(f"Filled {missing_charges} missing totalcharges with median: {median_charges}")
        ...
        for col in object_cols:
            ...
            try:
                ...
                if len(uniq_val) <= 2 and set(str(v).lower() for v in uniq_val) <= {'male', 'female'}:
                    ...
                    logger.info(f"Mapped gender column '{col}' to binary values")
                ...
                # Handle yes/no columns
                    ...
                    logger.info(f"Mapped yes/no column '{col}' to binary values")
        ...
        for col in df_features.select_dtypes(include=['category']).columns:
            ...
            logger.info(f"Converted category column '{col}' to numeric")
        ...
        for col in obj_col:
            try:
                ...
                logger.error(f"Error one-hot encoding column '{col}': {str(e)}")
        ...
        if is_training and target_col in clean_df.columns:
            ...
            logger.info(f"Stored {len(FEATURE_NAMES)} feature names for training")
        
        logger.info(f"Feature preprocessing completed. Output shape: {clean_df.shape}")
        return clean_df
...
def select_features(df, target_col='churn', k=10):
    ...
    try:
        logger.info(f"Starting feature selection with k={k}")
...
def get_feature_importance_names(df, target_col='churn', top_n=10):
    ...
    try:
        logger.info(f"Calculating feature importance for top {top_n} features")
        ...
        logger.info("Top Feature Importances calculated successfully")
...
def load_and_split_data(test_size=0.2, random_state=42):
    ...
    try:
        ...
        logger.error(f"Error loading and splitting data: {str(e)}")
        raise
...
def main():
    ...
    try:
        logger.info("=== Starting Features Module Demo ===")
        ...
        logger.info("=== Features Module Demo Completed Successfully ===")
